...
const idFormCarrito = document.getElementById("idFormCarrito")
const idProducto = document.getElementById("idProducto")



idFormCarrito.addEventListener('submit', (event) => {
    event.preventDefault() 
    console.log(event.target)

    let datForm = new FormData(event.target) 

    console.log(datForm.get("nombreProducto"))

    const carritos = new Producto(productos)
...
botonCarrito.addEventListener("click", () => {
    ...
    carritoStorage.forEach((producto, indice) =>{
        divCarrito.innerHTML += `
        <div class="card border-dark mb-3" id ="producto ${indice}" style="max-width: 20rem; margin: 4px;">
